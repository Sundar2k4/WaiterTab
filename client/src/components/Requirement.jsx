    ...
    if (dishname) {
      try {
        const response = await axios.post(
          "https://waitertab-server.onrender.com/disher",
          { dishname, dishcost }
        );
  ...
  const handledelete = async (id) => {
    try {
      const response = await axios.delete(
        `https://waitertab-server.onrender.com/dish-delete/${id}`
      );
  ...
  return (
    <div className="min-h-screen bg-prof flex flex-col items-center justify-center p-6">
      ...
      <div>
        <h1
          onClick={gotodash}
          className="cursor-pointer bg-black/10 rounded-md my-2 p-3 hover:scale-105 transform transition-all  duration-300 ease-in"
        >
          Al La Carte
          <svg
        ...
        {responses.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {responses.map((dish, index) => (
              <div
                key={index}
                className="bg-black/20 shadow-md rounded-lg p-4 text-center hover:scale-105 transform transition-all duration-300  ease-in-out"
              >
